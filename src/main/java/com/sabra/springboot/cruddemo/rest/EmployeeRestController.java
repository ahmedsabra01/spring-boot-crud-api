...
public class EmployeeRestController {
    ...
    public Employee updatePartialy(@PathVariable int employeeId, @RequestBody Map<String,Object> employeePreLoad){
        Employee tempEmployee = employeeService.findById(employeeId);

        if (tempEmployee == null){
            throw new RuntimeException("Employee Not Found with ID - "+ employeeId);
        }
        if (employeePreLoad.containsKey("id")){
            throw new RuntimeException("ID cannot be sent in the Request Body");
        }
        Employee patchedEmployee = jsonMapper.updateValue(tempEmployee,employeePreLoad);
